      ...
      while (1) {
         ...
         if (IS_OK(op_result)) {
            ...
            for (int pos = 0; pos < (int)count; ++pos) {
               _u32 a = nodes[pos].angle_z_q14 * 90.f / (1 << 14);
               _u32 b = nodes[pos].dist_mm_q2 / 4.0f;
               _u32 c = nodes[pos].quality;/*
               ...
               // send data to client
               try {
                  sock << std::to_string(a) << "," << std::to_string(b) << "," << std::to_string(c) << "\n";
               }
