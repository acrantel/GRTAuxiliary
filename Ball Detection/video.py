...
# Constants of camera and ball
kHorizontalFOVDeg = 62.8
kVerticalFOVDeg = 37.9

kBallHeightIn = 3.5
#kCameraHeightIn = 17.5
#kCameraPitchDeg = -10
kCameraHeightIn = 1
kCameraPitchDeg = 0
...
dist_coeffs = np.array([-0.15291752,  0.11312259,  0.0044382,   0.03888565, -0.04706539])
...
def distance_to_ball(knownWidth, focalLength, percievedWidth):
    return (knownWidth * focalLength) / percievedWidth
...
while cv2.waitKey(1) < 0:
    ...
    image = cv2.undistort(image, cameraMatrix, dist_coeffs) 
    ...
    for out in outs: 
        ...
        #each detection  has the form like this [center_x center_y width height obj_score class_1_score class_2_score ..]
            ...
            if confidence > 0.5:
                ...
                #print("left point: (%f, %f)" % (center_x - w / 2, center_y))
                #print("right point: (%f, %f)" % (center_x + w / 2, center_y))
                x_scale = 2 * (center_x / image.shape[1]) - 1
    ...
    #print("image0: " + str(image.shape[1]))
    #print("undist0: " + str(undist.shape[1]))
